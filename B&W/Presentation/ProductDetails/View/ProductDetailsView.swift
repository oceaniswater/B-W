...
struct ProductDetailsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 15) {
                AsyncImage(url: URL(string: vm.product?.imagePath ?? "")) { image in
                    image
                        .scaledToFit()
                ...
                FiveStarView(rating: Decimal(vm.product?.rating ?? 0.0), color: .yellow, backgroundColor: Color(uiColor: UIColor.systemGray5))
